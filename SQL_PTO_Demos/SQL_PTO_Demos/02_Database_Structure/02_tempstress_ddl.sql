...
--Step 5: Look for memory_optimized tables
USE tempdb;
go
...
--Step 7: Turn on Memory_Optimized TempDB_MetaData 
...
--Step 11: Check current activity of TempDB
USE master;
GO
SELECT 
er.session_id, er.wait_type, er.wait_resource,
...
--Step 12: Look for memory_optimized tables
USE tempdb;
go
SELECT OBJECT_NAME(object_id) as [object_name], * FROM sys.dm_db_xtp_object_stats;
go
